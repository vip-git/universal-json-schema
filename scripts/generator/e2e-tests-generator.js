...
const e2eTestsGenerator = (
  ...
  let xhrSchema = {};
  let uiSchema = {};
  const schema = require(`../../demo/examples/${pageName}/schema.json`);
  try {
    uiSchema = require(`../../demo/examples/${pageName}/ui-schema.json`);
  } catch (err) {}
  try {
    xhrSchema = require(`../../demo/examples/${pageName}/xhr-schema.json`);
  } catch (err) {}
  const formData = require(`../../demo/examples/${pageName}/form-data.json`);
  ...
  try {
    const testSchema = require(`../../demo/examples/${pageName}/tests-schema.json`);
